...
import static de.healthIMIS.iris.hd_server.core.SyncTimes.DataTypes.Requests;
...
class DataRequestSyncJob {
	...
	void run() {
		...
		var lastSync = syncTimes.findById(Requests).map(SyncTimes::getLastSync).orElse(LocalDateTime.of(2000, 01, 01, 00, 00));
		...
		try {
			...
			if (errorCounter == 1 || errorCounter % 20 == 0) {
				if (log.isTraceEnabled()) {
					log.warn(String.format("Request job - can't submit data requests to public server for %s tries!", errorCounter), e);
				} else {
	...
	private void submitDataRequest(DataRequest dataRequest) {
		...
		rest.put(
			"http://{address}:{port}/hd/data-requests/{id}",
			DataRequestInternalOutputDto.of(dataRequest),
	...
	static class DataRequestInternalOutputDto {
		...
		static DataRequestInternalOutputDto of(DataRequest request) {
			...
			return new DataRequestInternalOutputDto(
				request.getDepartmentId().toString(),
				request.getCheckCodeName(),
				request.getCheckCodeDayOfBirth(),
