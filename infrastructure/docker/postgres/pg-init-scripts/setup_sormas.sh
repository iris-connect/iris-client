...
if [ "${SORMAS_POSTGRES_USER}" != "$POSTGRES_USER" ]; then
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<EOSQL
    CREATE DATABASE ${SORMAS_DB_NAME} WITH OWNER = '${SORMAS_POSTGRES_USER}' ENCODING = 'UTF8';
    CREATE DATABASE ${SORMAS_DB_NAME_AUDIT} WITH OWNER = '${SORMAS_POSTGRES_USER}' ENCODING = 'UTF8';
    \c ${SORMAS_DB_NAME}
