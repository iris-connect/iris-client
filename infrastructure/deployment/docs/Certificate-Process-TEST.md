# Bestellung von Zertifikaten - TEST Umgebung

Folgen Sie dieser Anleitung für die Austellung von Zertifikaten für die TEST Umgebung.
...
Für die TEST Umgebung wird die Test-Domain `proxy.test-gesundheitsamt.de` vom IRIS Team bereit gestellt. Sie können sich dafür ein selbst signiertes Zertifikat ausstellen.
...
2. Erstellen Sie eine CSR Request
   ...
   ```
   ./scripts/test/create-csr-for-eps.sh
   ```
...
2. Erstellen Sie eine CSR Request
   ...
   ```
   ./scripts/test/create-csr-for-proxy.sh
   ```
