...
class EventDataRequestControllerIntegrationTest {
	...
	public void getDataRequests() throws Exception {
		var res =
			mockMvc.perform(MockMvcRequestBuilders.get("/data-requests-client/events")).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
	...
	public void updateDataRequestByCode() throws Exception {
		String REFID = "refId";
		var dataRequest = Mockito.spy(new EventDataRequest(REFID, "name", Instant.now(), Instant.now(), "", "requestDetails", "", new Location()));

		var dataRequestUpdated =
			new EventDataRequest(REFID, "name", Instant.now(), Instant.now(), "This is a test comment", "requestDetails", "", new Location());
		...
		mockMvc
			.perform(
				MockMvcRequestBuilders.patch("/data-requests-client/events/d1893f10-b6e3-11eb-8529-0242ac130003")
					.content(patchBody)
	...
	private void postNewDataRequest() throws Exception {
		var dataRequest = Mockito
			.spy(new EventDataRequest("refId", "name", Instant.now(), Instant.now(), "This is a test comment", "requestDetails", "", new Location()));
		...
		Mockito.when(dataRequestManagement.createDataRequest(any(), any(), any(), any(), any(), any(), any(), any(), any())).thenReturn(dataRequest);
