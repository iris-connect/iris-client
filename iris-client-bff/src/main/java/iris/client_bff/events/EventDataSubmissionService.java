...
public class EventDataSubmissionService {
	...
	public void deleteFailedSubmissionAttempt(EventDataRequest dataRequest, GuestList guestList) {
		submissions.findAllByRequest(dataRequest).get().findFirst().ifPresent(it -> submissions.delete(it));
		requestService.save(dataRequest);
	}
}
