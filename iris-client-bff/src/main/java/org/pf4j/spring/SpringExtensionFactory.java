...
public class SpringExtensionFactory implements ExtensionFactory {
	...
	protected <T> T createWithSpring(final Class<T> extensionClass, final ApplicationContext applicationContext) {
		...
		final Object autowiredExtension = beanFactory.autowire(extensionClass, AUTOWIRE_CONSTRUCTOR,
				// The value of the 'dependencyCheck' parameter is actually irrelevant as the using constructor of
				// 'RootBeanDefinition'
	 ...
	 * <ol>
	 * <li>If the given {@code extensionClass} belongs to a plugin that is a {@link SpringPlugin} the plugins context will
	 * be returned.</li>
	 * <li>Otherwise, if the given {@link #pluginManager} of this instance is a {@link SpringPluginManager} the managers
	 * context will be returned.</li>
	...
	protected <T> Optional<ApplicationContext> getApplicationContextBy(final Class<T> extensionClass) {
		...
		if (plugin instanceof SpringPlugin) {
			...
			log.debug(
					"  Extension class ' " + nameOf(extensionClass) + "' belongs to a non spring-plugin (or main application)" +
	...
	protected <T> T createWithoutSpring(final Class<T> extensionClass) throws IllegalArgumentException {
		...
		try {
			...
			throw new RuntimeException(
					"Most likely this exception is thrown because the called constructor (" + constructor + ")" +
